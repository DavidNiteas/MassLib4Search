...
- [`FragmentSearcher`](#fragment-searcher-anchor)：用于搜索碎片的Searcher类。
- [`EmbeddingSearcher](#embedding-searcher-anchor)：用于进行表征向量的搜索

### <a id="precursor-searcher-anchor"></a> PrecursorSearcher

### <a id="fragment-searcher-anchor"></a> FragmentSearcher

### <a id="embedding-searcher-anchor"></a> EmbeddingSearcher
...
- ~Search：搜索工具组，用于搜索质谱数据，提供配置化的计算参数管理。这种工具的一般输出为`Tuple[Index,Score]`，根据函数所针对的搜索任务不同，`Index`和`Score`的类型也不同：
  ...
  - 如果Search函数的输入参数中存在参数`topk`，则存在两种输出模式：
    - `top_k`模式：`Index`是一个二维张量，形状为`(num_queries, topk)`，其中存放这ref的索引；`Score`和`Index`完全对应，内部存放着每一个ref的分数。
    - `hit`模式：与不存在参数`topk`时类似，`Index`是一个二维张量，形状为`(num_hits, 3)`，0为query索引，1为ref索引，2为top_k索引；`Score`是一个一维张量，形状为`(num_hits)`，存放每一组hit的分数。
...
| output_device | Union[str, torch.device, Literal['auto']] | 'auto' | 结果存储设备（默认与计算设备一致） |
| operator_kwargs | Optional[dict] | None | 相似度计算算子的额外参数 |
| output_mode | Literal['top_k', 'hit'] | 'top_k' | 返回结果的格式 |
...
3. **hit模式输出**
   - 输出类型: `List[Tuple[torch.Tensor, torch.Tensor]]`
   - 形状: `num_hitted`个有效命中项
     - 每个元素的形状: `(num_hitted, 3)` 对于索引矩阵，`(num_hitted,)` 对于相似度分数矩阵
     - 每行格式: `[qry_index, ref_index, top_k_pos]` 对于索引矩阵
   - 数据类型: `torch.long` 对于索引矩阵，`torch.float32` 对于相似度分数矩阵
   - 设备位置: 默认与计算设备相同，可通过`output_device`参数指定
   - 示例:
     ```python
     # hit模式下，run()方法的输出
     indices, scores = embedding_similarity_search_tool.run(query, ref, output_mode='hit')
     indices.shape  # torch.Size([num_hitted, 3])
     scores.shape   # torch.Size([num_hitted,])

     # hit模式下，run_by_queue()方法的输出
     results = embedding_similarity_search_tool.run_by_queue(query_queue, ref_queue, output_mode='hit')
     results[0][0].shape  # 第一组结果的索引矩阵形状，torch.Size([num_hitted_1, 3])
     results[0][1].shape  # 第一组结果的相似度分数矩阵形状，torch.Size([num_hitted_1,])
     results[1][0].shape  # 第二组结果的索引矩阵形状，torch.Size([num_hitted_2, 3])
     results[1][1].shape  # 第二组结果的相似度分数矩阵形状，torch.Size([num_hitted_2,])
     ```
